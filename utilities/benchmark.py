...
class BenchmarkResult:
    ...
    error
        Truncated error message when :attr:`status` is not ``"SUCCEEDED"``.
    metadata
...
class BenchmarkRunner:
    ...
    def run_case(
        ...
        # ``torch-mps`` is the only backend with an asynchronous queue we
        # need to drain to time fairly; ``numpy`` / ``jax`` / ``torch-cpu``
        # are synchronous from the host's perspective.
