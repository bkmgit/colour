...
class MultiSignals(AbstractContinuousFunction):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Per-column :class:`Signal` views are materialised on demand by
    ...
    def signals(self) -> Mapping[str, Signal]:
        ...
        Per-column :class:`Signal` views are materialised lazily on
        first access; the canonical state lives at the parent level so
        the materialisation is a row-slice of ``self._range`` wrapped in
        a :class:`Signal` instance per label.
    ...
    def __setitem__(self, x: ArrayLike | slice, y: ArrayLike) -> None:
        ...
        def set_range(
            index: ArrayLike | slice, values: ArrayLike, xp: ModuleType
        ) -> None:
            """
    ...
    def arithmetical_operation(
        ...
        if in_place:
            if isinstance(a, MultiSignals):
                ...
                if not isinstance(self._range, np.ndarray) and isinstance(
                    operand, np.ndarray
                ):
                    xp = array_namespace(self._range)

                    operand = xp_as_array(operand, xp=xp, like=self._range)
                self.range = ioperator(self._range, operand)
