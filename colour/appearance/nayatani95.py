...
L_star_N=np.float64(75.8970185...))
    ...
    # ``xp.where`` promotes the *Python* float / int branches to *float64*;
    # cast back to keep the active backend dtype.
    eR = xp_as_float_array(xp.where((20 * xi) <= R, 1.758, 1), xp=xp, like=XYZ)
    eG = xp_as_float_array(xp.where((20 * eta) <= G, 1.758, 1), xp=xp, like=XYZ)
