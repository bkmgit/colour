...
def sd_multi_leds_Ohno2005(
    ...
    *Ohno (2005)* method.
    ...
    for peak_wavelength, half_spectral_width, peak_power_ratio in zip(
        ...
        # ``.item()`` works uniformly across NumPy, JAX, and PyTorch element
        # slices where ``float(...)`` is JAX-strict for ``(1,)``-shaped slices.
        sd += (
            sd_single_led_Ohno2005(
                peak_wavelength.item(), half_spectral_width.item(), **kwargs
            )
            * peak_power_ratio.item()
        )
