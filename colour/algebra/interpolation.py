...
\\sum_{x=-\\infty}^{\\infty}k(w_0 - x)\\cdot g(x)`
    ...
    def _evaluate(self, x: NDArrayFloat) -> NDArrayFloat:
        ...
        x = xp_atleast_1d(xp_as_float_array(x, xp=xp, like=self._x), xp=xp)
        ...
        windows = x_f[..., None] + xp_as_float_array(
            np.arange(-self._window + 1, self._window + 1), xp=xp, like=self._x
        )
        ...
        return xp.sum(
            xp_as_float_array(self._y_p, xp=xp, like=self._x)[windows] * weights,
            axis=1,
        )
...
class LinearInterpolator:
    ...
    def _evaluate(self, x: NDArrayFloat) -> NDArrayFloat:
        ...
        x = xp_as_float_array(x, xp=xp, like=self._x)
        ...
        if self._y.ndim == 1:
            return xp_interp(
                x,
                xp_as_float_array(self._x, xp=xp, like=self._x),
                self._y,
                xp=xp,
            )
        ...
        self_x = xp_as_float_array(self._x, xp=xp, like=self._x)
        ...
        x_low = xp_as_float_array(self_x_np[i_np], xp=xp, like=self._x)
        x_high = xp_as_float_array(self_x_np[i_np + 1], xp=xp, like=self._x)
        y_low = xp_as_float_array(self_y_np[i_np], xp=xp, like=self._x)
        y_high = xp_as_float_array(self_y_np[i_np + 1], xp=xp, like=self._x)
...
class SpragueInterpolator:
    ...
    *CIE* for interpolating functions with uniformly spaced independent
    ...
    def _evaluate(self, x: NDArrayFloat) -> NDArrayFloat:
        ...
        i = xp.searchsorted(xp_as_float_array(self._x_p, xp=xp, like=self._x), x) - 1
        ...
        with sdiv_mode():
            ...
            t = sdiv(
                x - xp_as_float_array(_x_p_np[i_np], xp=xp, like=self._x),
                xp_as_float_array(
                    _x_p_np[i_np + 1] - _x_p_np[i_np], xp=xp, like=self._x
                ),
            )
        ...
        r_s = xp.stack(
            [
                xp_as_float_array(r_np[i_np + k], xp=xp, like=self._x)
        ...
        powers = xp_as_float_array([[1], [2], [3], [4], [5]], xp=xp, like=self._x)
        ...
        if self._y.ndim == 2:
            basis = basis[..., None]
        return xp_as_float_array(r_np[i_np], xp=xp, like=self._x) + (a * basis).sum(
            axis=0
        )
...
class NullInterpolator:
    ...
    def _evaluate(self, x: NDArrayFloat) -> NDArrayFloat:
        ...
        sx = xp_as_float_array(self._x, xp=xp, like=self._x)
        sy = xp_as_float_array(self._y, xp=xp, like=self._x)
...
def table_interpolation_trilinear(V_xyz: ArrayLike, table: ArrayLike) -> NDArrayFloat:
    ...
    i_m = xp_as_int_array(table.shape[:-1], xp=xp) - 1
    V_xyz_s = V_xyz * i_m
