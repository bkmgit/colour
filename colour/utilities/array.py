...
_ARRAY_API_ENABLED: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "_ARRAY_API_ENABLED", default=False
)
"""
...
via :func:`set_array_api_enabled` or :class:`array_api_enable`.
"""
_ARRAY_API_ENABLED.set(as_bool(os.environ.get("COLOUR_SCIENCE__ARRAY_API", "False")))
...
def _resolve_backend_dtype(xp: ModuleType, dtype: Any) -> Any:
...
def as_ndarray(a: Any) -> np.ndarray:
    ...
    # The first two rungs raise :class:`TypeError` on backends without a
    # zero-copy ``__array__`` / *DLPack* hand-off; the *DLPack* rung also
    # raises :class:`RuntimeError` for device-resident tensors (notably
    # *PyTorch* on *MPS*: "Unsupported device in DLTensor") which is
    # equally recoverable via the ``.detach().cpu()`` / ``to_device``
    # rungs below.
    try:
        return np.asarray(a)
    except TypeError:
    ...
    # *PyTorch* tensors on a non-*CPU* device with ``requires_grad=True`` need
    # ``detach().cpu()`` before *DLPack* / ``__array__`` can succeed; for
    # other backends, ``array_namespace(a).to_device(a, "cpu")`` is the
    # *Array API* standard host hand-off.
    if hasattr(a, "detach") and hasattr(a, "cpu"):
        return np.asarray(a.detach().cpu())

    return np.asarray(array_namespace(a).to_device(a, "cpu"))
    ...
    *,
        ...
        if dtype is not None:
            ...
            if a_dtype is not None and a_dtype != dtype:
                ...
                if a_dtype != xp_target_dtype:
                    try:
                        result = xp_astype(a, xp_target_dtype, xp=xp)
                    except (TypeError, RuntimeError):
                        runtime_warning(
    ...
    if is_caching_enabled() and not copy:
        if isinstance(a, (int, float)):
            hash_key = hash((a, id(xp), str(device), dtype))
        elif isinstance(a, np.ndarray) and a.size <= 16:
            hash_key = hash(
                (int_digest(a.tobytes()), a.shape, id(xp), str(device), dtype)
            )

        if hash_key is not None:
            cached = _CACHE_SCALAR_PROMOTION.get(hash_key)
            if cached is not None:
                return cached
    ...
    try:
        result = (
            xp.asarray(a, device=device_kwarg)
    ...
    if dtype is not None and hasattr(result, "dtype"):
        xp_target_dtype = _resolve_backend_dtype(xp, dtype)
        if result.dtype != xp_target_dtype:
            try:
    ...
    if hash_key is not None:
        _CACHE_SCALAR_PROMOTION[hash_key] = result

    return result


def xp_as_float_array(
    a: ArrayLike, *, xp: ModuleType | None = None, like: Any = None
) -> NDArrayFloat:
    """
...
def xp_ascontiguousarray(a: ArrayLike, *, xp: ModuleType | None = None) -> NDArray:
    """
...
def xp_matrix_transpose(a: ArrayLike, *, xp: ModuleType | None = None) -> NDArray:
    """
...
def _runtime_warning_xp_downcast(xp: ModuleType, dtype: Any, dtype_target: Any) -> None:
...
@typing.overload
def xp_gradient(
    f: ArrayLike, *varargs: Any, xp: ModuleType | None = None, axis: int
) -> NDArrayFloat: ...
@typing.overload
def xp_gradient(
    f: ArrayLike, *varargs: Any, xp: ModuleType | None = None, axis: None = None
) -> NDArrayFloat | list[NDArrayFloat]: ...
def xp_gradient(
    f: ArrayLike, *varargs: Any, xp: ModuleType | None = None, axis: Any = None
) -> NDArrayFloat | list[NDArrayFloat]:
    """
...
def xp_resize(a: ArrayLike, new_shape: Any, *, xp: ModuleType | None = None) -> NDArray:
    """
...
def _scale_by(
    a: ArrayLike, factor: float, *, xp: ModuleType | None = None
) -> NDArrayFloat:
...
def xp_radians(a: ArrayLike, *, xp: ModuleType | None = None) -> NDArrayFloat:
    """
...
def xp_degrees(a: ArrayLike, *, xp: ModuleType | None = None) -> NDArrayFloat:
    """
...
def xp_reshape(a: ArrayLike, shape: Any, *, xp: ModuleType | None = None) -> NDArray:
    """
...
def xp_broadcast_to(
    a: ArrayLike, shape: Any, *, xp: ModuleType | None = None
) -> NDArray:
    """
...
def _xp_eig_generic(a: ArrayLike, xp: ModuleType, name: str) -> tuple[NDArray, NDArray]:
    ...
    try:
        ...
        return xp_as_array(w, xp=xp), xp_as_array(v, xp=xp)


def xp_eig(a: ArrayLike, *, xp: ModuleType | None = None) -> tuple[NDArray, NDArray]:
    """
...
def xp_eigh(a: ArrayLike, *, xp: ModuleType | None = None) -> tuple[NDArray, NDArray]:
    """
...
def xp_linspace(
    ...
    **kwargs
    ...
    try:
        return xp.linspace(start, stop, num, device=device, **kwargs)
    except (AttributeError, TypeError):
...
def xp_unique(
    a: ArrayLike, *, xp: ModuleType | None = None, **kwargs: Any
...
def xp_insert(
    ...
    *Array API* compatible implementation of :func:`numpy.insert` for sorted
    ...
    a = xp_as_array(a, xp=xp)
    indices = xp_as_array(indices, xp=xp)
    values = xp_as_array(values, xp=xp)
    ...
    # loop below.
    order = xp.argsort(indices)
    values = slice_along(values, order)
    indices = as_ndarray(indices[order])
    ...
    for i in range(indices.shape[0]):
        idx = int(indices[i])
        parts.append(slice_along(a, slice(prev, idx)))
        parts.append(slice_along(values, slice(i, i + 1)))
        prev = idx
    parts.append(slice_along(a, slice(prev, a_axis)))
...
def xp_setxor1d(
    ...
    *Array API* compatible implementation of :func:`numpy.setxor1d`.
    ...
    # ``torch.sort`` returns a ``(values, indices)`` tuple; the *Array
    # API* standard's ``xp.sort`` returns the values array. Tolerate both
    # so this helper works with raw and wrapped namespaces.
...
def interval(distribution: ArrayLike, unique: bool = True) -> NDArray:
    ...
    if is_caching_enabled() and hash_key in _CACHE_DISTRIBUTION_INTERVAL:
        return xp_as_array(_CACHE_DISTRIBUTION_INTERVAL[hash_key], xp=xp, copy=True)
    ...
    _CACHE_DISTRIBUTION_INTERVAL[hash_key] = xp_as_array(interval_, xp=xp, copy=True)
